...
if($isHidden && !(is_user_logged_in())):
    ...
    if($hasBgShape): ?>
        <div class="svg__shape svg__shape-line z-3 d-none d-xl-block"><div class="inner" style="background: url(<?php echo get_template_directory_uri(); ?>/assets/img/storytelling-line.svg) no-repeat;"></div></div>
    ...
    <div class="cont pos-rel z-5 <?php echo $design['width']; ?>">
        <div class="inner inner__top pos-rel z-5 mb-5">
            <?php if(!empty($text['body'])): ?>
                <div class="row">
                    <div class="content-area col-lg-6 mb-4 mb-lg-0">
                        <?php echo acfOutput($text['body'], 'div', 'pps__text'); ?>
                        ...
                        <?php if($text['image_top']): ?>
                            <div class="image-area col-lg-6 text-center">
                                <div class="image__container"><?php echo acfImgOutput($text['image_top'], '', 'image__inner-top'); ?></div>
                            </div>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
        ...
        <div class="cont is-sm pos-rel z-5 text-center is-text--white">
            <div class="inner inner__bottom pos-rel z-5">
                <?php if(!empty($text['headline'])): ?>
                    <?php if($text['image_bottom_icon']): ?>
                        <div class="image__container mb-3 mb-lg-4"><?php echo acfImgOutput($text['image_bottom_icon'], '', 'image__inner-bottom-icon mx-auto'); ?></div>
                    <?php endif; ?>
                    <?php echo acfOutput($text['headline'], 'h2', 'pps__heading'); ?>
                    <?php echo acfOutput($text['subheadline'], 'h3', 'pps__subheading font-weight-normal'); ?>
                    <a class="btn btn--primary is-green guide-people" href="<?php echo $text['link_url_2']; ?>"><?php echo $text['link_text']; ?></a>
                    <!-- <p class="mb-0 mt-4"><button class="btn btn-none btn-link is-text--green-10 pps__link" type="button" data-toggle="modal" data-target="#people-modal"><?php echo $text['link_text']; ?></button></p> -->

                <?php endif; ?>
            </div>
        </div>
        <?php if($text['image_bottom']): ?>
            <div class="image__container image__inner-bottom pos-rel z-3"><?php echo acfImgOutput($text['image_bottom'], '', 'image__inner-bottom-img'); ?></div>
        <?php endif; ?>
        <?php
        echo '</section>'; ?>
        ...
        if ( $loop->have_posts() ) : ?>
            <aside class="modal modal__popup modal__full fade people-modal" id="people-modal" tabindex="-1" role="dialog" aria-labelledby="people-modal" aria-hidden="true">
